...
if(isMultipart)
{
    ...
    while(itr.hasNext())//checking if there is any value in the position like rs.next()
    {
       ...
       else//when it is an image or multipart
       {
           ...
           String ext=file_name.substring(file_name.lastIndexOf("."));//means the extension is stored to variable ext
            ...
            String file_path=proj_path.getParentFile().getParentFile().getPath()+"\\Web\\Musician\\Photo\\";//path for storing image
   ...
   String ins="insert into tbl_bandmember(bandmember_name,bandmember_gender,bandmember_dob,bandmember_address,city_id,bandmember_contact,bandmember_email,band_id,artsub_id,bandmember_photo,bandmember_username,bandmember_password)values('"+value[0]+"','"+value[1]+"','"+value[2]+"','"+value[3]+"','"+value[5]+"','"+value[6]+"','"+value[7]+"','"+session.getAttribute("bandid")+"','"+value[9]+"','"+fn+"','"+value[10]+"','"+value[11]+"')";
    ...
    if(status==true)
    {
      %> 
       <script type="text/javascript">
            ...
            setTimeout(function(){window.location.href='AddBandMembers.jsp'},40);//40millisecend it go to next page
        </script>
     <%
    }
     }    
