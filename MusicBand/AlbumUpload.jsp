...
if(isMultipart)
{
    ...
    while(itr.hasNext())//checking if there is any value in the position like rs.next()
    {
       ...
       else//when it is an image or multipart
       {
           ...
           String ext=file_name.substring(file_name.lastIndexOf("."));//means the extension is stored to variable ext
            ...
            String file_path=proj_path.getParentFile().getParentFile().getPath()+"\\Web\\MusicBand\\Album\\";//path for storing image
            Random r=new Random();//to generate random number
             int r_num=r.nextInt(1111)+999;//values between this are generated
             
            fn="Album"+r_num+ext;//generating random number for images so this name and format of imagewil be stored inside the server
   ...
   String ins="insert into tbl_album(album_name,album_date,album_time,album_poster,album_description,album_location,band_id)values('"+value[0]+"','"+value[1]+"','"+value[2]+"','"+fn+"','"+value[4]+"','"+value[3]+"','"+session.getAttribute("bandid")+"')";
    ...
    if(status==true)
    {
      %> 
       <script type="text/javascript">
            ...
            setTimeout(function(){window.location.href='AlbumLaunch.jsp'},40);//40millisecend it go to next page
        </script>
     <%
    }
     }    
