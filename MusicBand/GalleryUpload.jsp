...
if(isMultipart)
{
    ...
    while(itr.hasNext())//checking if there is any value in the position like rs.next()
    {
       ...
       else//when it is an image or multipart
       {
           ...
           String ext=file_name.substring(file_name.lastIndexOf("."));//means the extension is stored to variable ext
            ...
            String file_path=proj_path.getParentFile().getParentFile().getPath()+"\\Web\\Music Band\\Band Gallery\\";//path for storing image
   ...
   String ins="insert into tbl_bandgallery(band_file,band_desc,band_id)values('"+fn+"','"+value[0]+"','"+session.getAttribute("bandid")+"')";
    ...
    if(status==true)
    {
      %> 
       <script type="text/javascript">
            ...
            setTimeout(function(){window.location.href='BandGallery.jsp'},40);//40millisecend it go to next page
        </script>
     <%
    }
     }    
