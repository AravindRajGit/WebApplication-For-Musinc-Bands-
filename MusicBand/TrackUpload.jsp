...
if(isMultipart)
{
    ...
    while(itr.hasNext())//checking if there is any value in the position like rs.next()
    {
       ...
       else//when it is an image or multipart
       {
           ...
           String ext=file_name.substring(file_name.lastIndexOf("."));//means the extension is stored to variable ext
            ...
            String file_path=proj_path.getParentFile().getParentFile().getPath()+"\\Web\\Music Band\\TrackPhoto\\";//path for storing image
   ...
   String ins="insert into tbl_tracks(track_name,genre_id,band_id,track_detail,track_price,track_photo)values('"+value[0]+"','"+value[1]+"','"+session.getAttribute("bandid")+"','"+value[2]+"','"+value[3]+"','"+fn+"')";
    ...
    if(status==true)
    {
      %> 
       <script type="text/javascript">
            ...
            setTimeout(function(){window.location.href='Tracks.jsp'},40);//40millisecend it go to next page
        </script>
     <%
    }
     }    
