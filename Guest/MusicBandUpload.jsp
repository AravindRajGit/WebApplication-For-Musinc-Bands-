...
if(isMultipart)
{
    ...
    while(itr.hasNext())//checking if there is any value in the position like rs.next()
    {
       ...
       else//when it is an image or multipart
       {
           ...
           String ext=file_name.substring(file_name.lastIndexOf("."));//means the extension is stored to variable ext
            ...
            String file_path=proj_path.getParentFile().getParentFile().getPath()+"\\Web\\Music Band\\Logo\\";//path for storing image
            Random r=new Random();//to generate random number
             int r_num=r.nextInt(1111)+999;//values between this are generated
             
            fn="logo"+r_num+ext;//generating random number for images so this name and format of imagewil be stored inside the server
   ...
   String ins="insert into tbl_musicband(band_name,band_year,district_id,band_contact,band_email,band_membersnum,band_logo,license_id,band_lproof,band_username,band_password)values('"+value[0]+"','"+value[1]+"','"+value[2]+"','"+value[3]+"','"+value[4]+"','"+value[5]+"','"+fn+"','"+value[6]+"','"+proof+"','"+value[7]+"','"+value[8]+"')";
    ...
    if(status==true)
    {
      %> 
       <script type="text/javascript">
            ...
            setTimeout(function(){window.location.href='MusicBandReg.jsp'},40);//40millisecend it go to next page
        </script>
     <%
    }
     }    
