...
if(isMultipart)
{
    ...
    while(itr.hasNext())//checking if there is any value in the position like rs.next()
    {
       ...
       else//when it is an image or multipart
       {
           ...
           String ext=file_name.substring(file_name.lastIndexOf("."));//means the extension is stored to variable ext
            ...
            String file_path=proj_path.getParentFile().getParentFile().getPath()+"\\Web\\Guest\\UserImages\\";//path for storing image
            Random r=new Random();//to generate random number
             int r_num=r.nextInt(1111)+999;//values between this are generated
             
            fn="User"+r_num+ext;//generating random number for images so this name and format of imagewil be stored inside the server
   ...
   String ins="insert into tbl_user(user_name,user_gender,user_dob,user_address,city_id,user_contact,user_email,user_photo,user_username,user_password)values('"+value[0]+"','"+value[1]+"','"+value[2]+"','"+value[3]+"','"+value[6]+"','"+value[7]+"','"+value[8]+"','"+fn+"','"+value[9]+"','"+value[10]+"')";
    ...
    if(status==true)
    {
      %> 
       <script type="text/javascript">
            ...
            setTimeout(function(){window.location.href='UserRegistration.jsp'},40);//40millisecend it go to next page
        </script>
     <%
    }
     }    
