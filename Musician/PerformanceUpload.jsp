...
if(isMultipart)
{
   ...
   Iterator itr=items_list.iterator();
    ...
    while(itr.hasNext())//checking if there is any value in the position like rs.next()
    {
       ...
       else//when it is an image or multipart
       {
           ...
           if(field_name.equals("filevideo"))//write the name of image here it is img_product
           {
           String ext=file_name.substring(file_name.lastIndexOf("."));//means the extension is stored to variable ext
            //setting path to store image
            File proj_path=new File(config.getServletContext().getRealPath("/"));
            String file_path=proj_path.getParentFile().getParentFile().getPath()+"\\Web\\Musician\\Performance Video\\";//path for storing image
            Random r=new Random();//to generate random number
             int r_num=r.nextInt(1111)+999;//values between this are generated
             
            fn="video"+r_num+ext;//generating random number for images so this name and format of imagewil be stored inside the server
   ...
   String ins="insert into tbl_musicianperformance(musician_id,performance_file,performance_caption)values('"+session.getAttribute("musicianid")+"','"+fn+"','"+value[0]+"')";
    ...
    if(status==true)
    {
      %> 
       <script type="text/javascript">
            ...
            setTimeout(function(){window.location.href='Performance.jsp'},40);//40millisecend it go to next page
        </script>
     <%
    }
     }    
